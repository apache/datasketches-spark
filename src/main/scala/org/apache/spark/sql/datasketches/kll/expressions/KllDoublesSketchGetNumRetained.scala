...
@ExpressionDescription(
  usage = """
    _FUNC_(expr) - Returns the number of items retained by the sketch given the binary representation
...
case class KllDoublesSketchGetNumRetained(sketchExpr: Expression)
 ...
 with NullIntolerant {
  ...
  override protected def withNewChildInternal(newChild: Expression): KllDoublesSketchGetNumRetained = {
