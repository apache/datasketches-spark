...
@ExpressionDescription(
  usage = """
    _FUNC_(expr) - Returns true if the sketch is in estimation mode, otherwise false given
...
case class KllDoublesSketchIsEstimationMode(sketchExpr: Expression)
 ...
 with NullIntolerant {
  ...
  override protected def withNewChildInternal(newChild: Expression): KllDoublesSketchIsEstimationMode = {
